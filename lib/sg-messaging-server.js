...
sgMessagingServer.prototype.publish = function (jobName, jobInput, options, callback) {
	...
	var priority = 'normal';
	if(options.priority && ~this.priorities.indexOf(options.priority)) {
		priority = options.priority;
	}

	var attempts = 1;
	if(options.attempts && typeof options.attempts === 'number' && options.attempts > 1) {
	...
	if(options.delay && typeof options.delay === 'number' && options.delay > 5) {
